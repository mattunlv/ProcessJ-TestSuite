public class Main9 {
	...
	static final RuntimeSystem rt = new RuntimeSystem(2);
	...
	private abstract class _p1 extends Process {
		...
		public int g() {
			...
			result = (_activation.getLocal(1) == null ? 0 : (int) _activation
					.getLocal(1));
			...
			case 0:
				if (param$c.isReadyToRead(this)) {
					...
					setNotReady();
					yield(new Activation(new Object[] { param$c,
							new Integer(result) }, 2), 0);
					return 0;
				}
				terminate();
				return result;
		...
		void p1() {
			...
			switch (runLabel) {
			case 0:
				a = (_activation.getLocal(1) == null ? 72
						: (Integer) _activation.getLocal(1));
			...
			case 1:
				...
				if (yielded) {
					...
					yield(new Activation(new Object[] { c, new Integer(a) }, 2),
							1);
					return;
				}
				terminate();
	...
	private abstract class _main extends Process {
		public _main(int n) {
			addActivationFirst(new Activation(new Object[] { new Integer(n) },
					2));
		}
		...
		public void run() {
			...
			int param$n;
			param$n = (_activation.getLocal(0) == null ? 0
			...
			case 0:
				...
				yield(new Activation(new Object[] { new Integer(param$n), c },
						2), 1);
				return;
			case 1:
				terminate();
	...
	public static void main(String args[]) {
		new Main9().run(Integer.parseInt(args[0]));
	}
}